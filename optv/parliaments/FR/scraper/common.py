#! /usr/bin/env python3
"""Shared HTTP helpers for the FR scrapers.

The assemblee-nationale.fr web front rate-limits aggressive access (429s), so
all fetches go through :func:`http_get` which retries with exponential backoff
and a polite User-Agent. The open-data hosts (``data.assemblee-nationale.fr``,
``www.assemblee-nationale.fr/dyn/opendata``) are friendlier than the dynamic
``/dyn/`` web pages, but we treat them uniformly.
"""

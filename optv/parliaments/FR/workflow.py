#! /usr/bin/env python3
"""FR (Assemblée nationale) workflow.

One source carries the text **and** the per-speech video offsets, plus one
external video URL per séance:

- **Syceron compte rendu** (per-séance XML) → verbatim text + agenda +
  per-speech ``stime`` offsets + ``id_acteur`` speakers (the merge spine).
- **interventions-video index → CRV page** → the séance's HLS video URL,
  resolved via the réunion id shared with the compte rendu.
- **AMO10 acteurs/organes ⋈ Wikidata** → ``entities.json`` (NEL) +
  ``acteurs.json`` (the député→groupe map the proceedings parser needs).

Stage orchestration (merge → NEL → align → NER → publish) is shared via
``optv.shared.workflow``. NER runs in French (entity-fishing ``fr`` + spaCy
``fr_core_news_md``); the French KB (``db-fr``) must be loaded into the
entity-fishing instance — see the README.

Period semantics: ``--period`` is the legislature (``17``). Session keys are the
Syceron compte-rendu uid suffix (e.g. ``2026O1N232``); see ``common.py`` for the
uid ↔ ``session.number`` encoding. Download takes explicit ``--fr-session`` keys
or, when omitted, the whole legislature's Syceron archive.
"""
...
def _session_in_scope(args, session):
    """Session keys are Syceron uids (``YYYY[OE]NnNNN``). Every key we hold is
    from the requested legislature's archive, so scope is just key validity plus
    the ``--limit-session`` filter."""
    try:
...
def _add_arguments(parser):
    ...
    parser.add_argument("--session", dest="fr_session", action="append", default=[],
                        help="Session key to process (e.g. 2026O1N232). Repeatable. "
                             "When omitted, the whole legislature archive is fetched.")


def main():
    run_main(
        PARLIAMENT_ID, HOOKS,
        description="FR Assemblée nationale workflow: download → parse → merge → "
                    "NEL → align → NER.",

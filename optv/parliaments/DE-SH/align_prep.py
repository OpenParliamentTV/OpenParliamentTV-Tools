#! /usr/bin/env python3
"""Stage per-speech DE-SH audio into per-speech MP3s for aeneas.

Each DE-SH speech is a ``<session-mp4>#t=start,end`` media fragment from the m7k
mediathek: one base MP4 per session, sliced per speech (video-relative offsets).
The shared driver downloads the session MP4's audio once and re-encode-slices
